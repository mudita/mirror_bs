ifndef MK_FLAGS_COMPILER_OPTIMIZATION_MK
MK_FLAGS_COMPILER_OPTIMIZATION_MK=	TRUE

# TODO: Flags should be parsed and checked it there is no conflicting options.
# INFO: Official documentation:
#       https://gcc.gnu.org/onlinedocs/gcc/Debugging-Options.html
#
# INFO: These options control various sorts of optimizations.
#
...
#       out the exact set of optimizations that are enabled at each level. See
...
#       standard-compliant programs. It turns on:
#
#              -ffast-math
#
#       and the Fortran-specific:
#
#              -fno-protect-parens
#              -fstack-arrays. 
#
...
#
#       This option is enabled by default at optimization levels -O, -O2, -O3,
#       -Os. 
#
# INFO: -ffp-contract=style
...
#
#       The default setting (when not optimizing for size) for 32-bit GNU/Linux
#       x86 and 32-bit Darwin x86 targets is -fomit-frame-pointer. You can
#       configure GCC with the --enable-frame-pointer configure option to
#       change the default.
#
#       Enabled at levels -O, -O2, -O3, -Os. 
#
# INFO: -foptimize-sibling-calls
#       Optimize sibling and tail recursive calls.
#
#       Enabled at levels -O2, -O3, -Os. 
#
# INFO: -foptimize-strlen
#       Optimize various standard C string functions (e.g. strlen, strchr or
#       strcpy) and their _FORTIFY_SOURCE counterparts into faster
#       alternatives.
#
...
#       be worth integrating in this way. This inlining applies to all
#       functions, even those not declared inline.
#
#       Enabled at level -O2. 
#
# INFO: -findirect-inlining
...
#       Enabled at level -O3. 
#
# INFO: -finline-functions-called-once
...
#       specified individually by using --param name=value. The
...
#              is set to n/2.
#
#       See below for a documentation of the individual parameters controlling
#       inlining and for the defaults of these parameters.
#
#       Note: there may be no value to -finline-limit that results in default
#       behavior.
#
#       Note: pseudo instruction represents, in this particular context, an
#       abstract measurement of function's size. In no way does it represent a
#       count of assembly instructions and as such its exact meaning might
#       change from one release to an another. 
#
# INFO: -fno-keep-inline-dllexport
...
#       This switch does not affect functions using the extern inline extension
#       in GNU C90. In C++, emit any and all inline functions into the object
#       file. 
#
# INFO: -fkeep-static-consts
...
#       using this option results in non-conforming behavior. 
#
# INFO: -fmodulo-sched
...
#
# INFO: -fmodulo-sched-allow-regmoves
#       Perform more aggressive SMS-based modulo scheduling with register moves
#       allowed. By setting this flag certain anti-dependences edges are
#       deleted, which triggers the generation of reg-moves based on the
#       life-range analysis. This option is effective only with -fmodulo-sched
#       enabled. 
#
# INFO: -fno-branch-count-reg
...
#       compare it against zero, then branch based upon the result. This option
...
#
# INFO: -fno-zero-initialized-in-bss
#       If the target supports a BSS section, GCC by default puts variables
#       that are initialized to zero into BSS. This can save space in the
#       resulting code.
#
#       This option turns off this behavior because some programs explicitly
#       rely on variables going to the data section—e.g., so that the resulting
#       executable can find the beginning of that section and/or make
...
#       independently. This normally generates better code for those types, but
#       may make debugging more difficult.
#
#       Enabled at levels -O, -O2, -O3, -Os. 
#
# INFO: -fcse-follow-jumps
...
#       common subexpression elimination. This pass attempts to move stores out
#       of loops. When used in conjunction with -fgcse-lm, loops containing a
...
#       pass eliminates redundant loads that come after stores to the same
#       memory location (both partial and full redundancies).
#
#       Not enabled at any optimization level. 
#
# INFO: -fgcse-after-reload
#       When -fgcse-after-reload is enabled, a redundant load elimination pass
#       is performed after reload. The purpose of this pass is to clean up
#       redundant spilling. 
#
# INFO: -faggressive-loop-optimizations
#       This option tells the loop optimizer to use language constraints to
#       derive bounds for the number of iterations of a loop. This assumes that
#       loop code does not invoke undefined behavior by for example causing
#       signed integer overflows or out-of-bound array accesses. The bounds for
#       the number of iterations of a loop are used to guide loop unrolling and
#       peeling and loop exit test optimizations. This option is enabled by
#       default. 
#
# INFO: -funsafe-loop-optimizations
...
#       you use -Wunsafe-loop-optimizations, the compiler warns you if it finds
#       this kind of loop. 
#
# INFO: -fcrossjumping
...
#       instructions to support this. Enabled by default at -O and higher on
...
#       instructions, and some tricks doable by standard arithmetics. The use
#       of conditional execution on chips where it is available is controlled
#       by -fif-conversion2.
#
#       Enabled at levels -O, -O2, -O3, -Os. 
#
# INFO: -fif-conversion2
#       Use conditional execution (where available) to transform conditional
#       jumps into branch-less equivalents.
#
#       Enabled at levels -O, -O2, -O3, -Os. 
#
# INFO: -fdeclone-ctor-dtor
#       The C++ ABI requires multiple entry points for constructors and
#       destructors: one for a base subobject, one for a complete object, and
#       one for a virtual destructor that calls operator delete afterwards. For
#       a hierarchy with virtual bases, the base and complete variants are
...
#       Enabled by -Os. 
#
# INFO: -fdelete-null-pointer-checks
...
#
# INFO: -fdevirtualize-speculatively
#       Attempt to convert calls to virtual functions to speculative direct
#       calls. Based on the analysis of the type inheritance graph, determine
#       for a given call the set of likely targets. If the set is small,
#       preferably of size 1, change the call into a conditional deciding
...
#
# INFO: -flive-range-shrinkage
#       Attempt to decrease register pressure through register live range
#       shrinkage. This is helpful for fast processors with small or moderate
#       size register sets. 
#
# INFO: -fira-algorithm=algorithm
...
#
# INFO: -fira-region=region
#       Use specified regions for the integrated register allocator. The region
#       argument should be one of the following:
#
#       ‘all’
#              Use all loops as register allocation regions. This can give the
#       best results for machines with a small and/or irregular register set. 
#       ‘mixed’
#              Use all loops except for loops with small register pressure as
#       the regions. This value usually gives the best results in most cases
#       and for most architectures, and is enabled by default when compiling
#       with optimization for speed (-O, -O2, ...). 
#       ‘one’
#              Use all functions as a single region. This typically results in
#       the smallest code size, and is enabled by default for -Os or -O0.
#
# INFO: -fira-hoist-pressure
...
#       and smaller code on machines with large register files
#       (>= 32 registers), but it can slow the compiler down.
#
#       This option is enabled at level -O3 for some targets. 
#
# INFO: -fno-ira-share-save-slots
#       Disable sharing of stack slots used for saving call-used hard registers
#       living through a call. Each hard register gets a separate stack slot,
#       and as a result function stack frames are larger. 
#
# INFO: -fno-ira-share-spill-slots
...
#       allocator. The default value is 5. If the value n is greater or equal
#       to 10, the dump output is sent to stderr using the same format as n
#       minus 10. 
#
# INFO: -flra-remat
...
#       Enabled at levels -O, -O2, -O3, -Os. 
#
# INFO: -fschedule-insns
#       If supported for the target machine, attempt to reorder instructions to
#       eliminate execution stalls due to required data being unavailable. This
#       helps machines that have slow floating point or memory load
...
#
# INFO: -fsched-pressure
#       Enable register pressure sensitive insn scheduling before register
#       allocation. This only makes sense when scheduling before register
#       allocation is enabled, i.e. with -fschedule-insns or at -O2 or higher.
#       Usage of this option can improve the generated code and decrease its
#       size by preventing register pressure increase above the number of
#       available hard registers and subsequent spills in register allocation. 
#
# INFO: -fsched-spec-load
#       Allow speculative motion of some load instructions. This only makes
#       sense when scheduling before register allocation, i.e. with
#       -fschedule-insns or at -O2 or higher. 
#
# INFO: -fsched-spec-load-dangerous
#       Allow speculative motion of more load instructions. This only makes
#       sense when scheduling before register allocation, i.e. with
#       -fschedule-insns or at -O2 or higher. 
#
# INFO: -fsched-stalled-insns
# INFO: -fsched-stalled-insns=n
...
#
# INFO: -fsched-stalled-insns-dep
# INFO: -fsched-stalled-insns-dep=n
#       Define how many insn groups (cycles) are examined for a dependency on a
#       stalled insn that is a candidate for premature removal from the queue
#       of stalled insns. This has an effect only during the second scheduling
#       pass, and only if -fsched-stalled-insns is used.
...
#
# INFO: -fsched-critical-path-heuristic
...
#
# INFO: -fsched-last-insn-heuristic
#       Enable the last-instruction heuristic in the scheduler. This heuristic
#       favors the instruction that is less dependent on the last instruction
#       scheduled. This is enabled by default when scheduling is enabled, i.e.
#       with -fschedule-insns or -fschedule-insns2 or at -O2 or higher. 
#
# INFO: -fsched-dep-count-heuristic
#       Enable the dependent-count heuristic in the scheduler. This heuristic
#       favors the instruction that has more instructions depending on it. This
#       is enabled by default when scheduling is enabled, i.e. with
#       -fschedule-insns or -fschedule-insns2 or at -O2 or higher. 
#
# INFO: -freschedule-modulo-scheduled-loops
#       Modulo scheduling is performed before traditional scheduling. If a loop
#       is modulo scheduled, later scheduling passes may change its schedule.
#       Use this option to control that behavior. 
#
# INFO: -fselective-scheduling
#       Schedule instructions using selective scheduling algorithm. Selective
#       scheduling runs instead of the first scheduler pass. 
#
# INFO: -fselective-scheduling2
#       Schedule instructions using selective scheduling algorithm. Selective
#       scheduling runs instead of the second scheduler pass. 
#
# INFO: -fsel-sched-pipelining
...
#
# INFO: -fsel-sched-pipelining-outer-loops
...
#       compiler cannot perform interprocedural propagation, inlining and other
#       optimizations in anticipation that the function or variable in question
#       may change. While this feature is useful, for example, to rewrite
#       memory allocation functions by a debugging implementation, it is
#       expensive in the terms of code quality. With
#       -fno-semantic-interposition the compiler assumes that if interposition
#       happens for functions the overwriting function will have precisely the
...
#       is never allowed for interposition to change semantics) and for symbols
...
#       Enabled at levels -O2, -O3, -Os. 
#
# INFO: -fcombine-stack-adjustments
...
#       used by any called function. In that case it is not necessary to save
#       and restore them around calls. This is only possible if called
#       functions are part of same compilation unit as current function and
#       they are compiled before it.
#
#       Enabled at levels -O2, -O3, -Os. 
#
# INFO: -fconserve-stack
#       Attempt to minimize stack usage. The compiler attempts to use less
#       stack space, even if that makes the program slower. This option implies
#       setting the large-stack-frame parameter to 100 and the
...
#
# INFO: -ftree-phiprop
#       Perform hoisting of loads from conditional pointers on trees. This pass
#       is enabled by default at -O and higher. 
#
# INFO: -fhoist-adjacent-loads
#       Speculatively hoist loads from both branches of an if-then-else if the
#       loads are from adjacent locations in the same structure and the target
#       architecture has a conditional move instruction. This flag is enabled
#       by default at -O2 and higher. 
#
# INFO: -ftree-copy-prop
#       Perform copy propagation on trees. This pass eliminates unnecessary
#       copy operations. This flag is enabled by default at -O and higher. 
#
# INFO: -fipa-pure-const
#       Discover which functions are pure or constant. Enabled by default at -O
#       and higher. 
#
# INFO: -fipa-reference
...
#       GCC ICF works on different levels and thus the optimizations are not
#       same - there are equivalences that are found only by GCC and
#       equivalences found only by Gold.
#
#       This flag is enabled by default at -O2 and -Os. 
#
# INFO: -fisolate-erroneous-paths-dereference
#       Detect paths that trigger erroneous or undefined behavior due to
#       dereferencing a null pointer. Isolate those paths from the main control
#       flow and turn the statement with erroneous or undefined behavior into a
#       trap. This flag is enabled by default at -O2 and higher and depends on
#       -fdelete-null-pointer-checks also being enabled. 
#
# INFO: -fisolate-erroneous-paths-attribute
...
#
# INFO: -ftree-switch-conversion
#       Perform conversion of simple initializations in a switch to
#       initializations from a scalar array. This flag is enabled by default at
#       -O2 and higher. 
#
# INFO: -ftree-tail-merge
#       Look for identical code sequences. When found, replace one with a jump
#       to the other. This optimization is known as tail merging or cross
#       jumping. This flag is enabled by default at -O2 and higher. The
#       compilation time in this pass can be limited using
#       max-tail-merge-comparisons parameter and max-tail-merge-iterations
#       parameter. 
#
# INFO: -ftree-dce
...
#       specified. 
#
# INFO: -ftree-dominator-opts
#       Perform a variety of simple scalar cleanups (constant/copy propagation,
#       redundancy elimination, range propagation and expression
#       simplification) based on a dominator tree traversal. This also performs
#       jump threading (to reduce jumps to jumps). This flag is enabled by
#       default at -O and higher. 
#
# INFO: -ftree-dse
#       Perform dead store elimination (DSE) on trees. A dead store is a store
#       into a memory location that is later overwritten by another store
...
#
# INFO: -ftree-loop-optimize
#       Perform loop optimizations on trees. This flag is enabled by default at
#       -O and higher. 
#
# INFO: -ftree-loop-linear
# INFO: -floop-interchange
# INFO: -floop-strip-mine
# INFO: -floop-block
# INFO: -floop-unroll-and-jam
...
#
# INFO: -floop-nest-optimize
#       Enable the ISL based loop nest optimizer. This is a generic loop nest
#       optimizer based on the Pluto optimization algorithms. It calculates a
#       loop structure optimized for data-locality and parallelism. This option
#       is experimental. 
#
# INFO: -floop-parallelize-all
...
#       contain loop carried dependences without checking that it is profitable
#       to parallelize the loops. 
#
# INFO: -ftree-coalesce-vars
...
#       SSA coalescing of user variables. This option is enabled by default if
#       optimization is enabled, and it does very little otherwise. 
#
# INFO: -ftree-loop-if-convert
...
#       pass to handle these loops. This is enabled by default if vectorization
#       is enabled. 
#
# INFO: -ftree-loop-if-convert-stores
...
#              ENDDO
#
# INFO: -ftree-loop-distribute-patterns
...
#       expand to nontrivial sequences of insns). With -funswitch-loops it also
#       moves operands of conditions that are invariant out of the loop, so
#       that we can use just trivial invariantness analysis in loop
#       unswitching. The pass also includes store motion. 
#
# INFO: -ftree-loop-ivcanon
...
#
# INFO: -ftree-parallelize-loops=n
...
#       related expressions involving multiplications and replaces them by less
#       expensive calculations when possible. This is enabled by default at -O
#       and higher. 
#
# INFO: -ftree-vectorize
#       Perform vectorization on trees. This flag enables -ftree-loop-vectorize
#       and -ftree-slp-vectorize if not explicitly specified. 
#
# INFO: -ftree-loop-vectorize
#       Perform loop vectorization on trees. This flag is enabled by default at
#       -O3 and when -ftree-vectorize is enabled. 
#
# INFO: -ftree-slp-vectorize
#       Perform basic block vectorization on trees. This flag is enabled by
#       default at -O3 and when -ftree-vectorize is enabled. 
#
# INFO: -fvect-cost-model=model
#       Alter the cost model used for vectorization. The model argument should
#       be one of ‘unlimited’, ‘dynamic’ or ‘cheap’. With the ‘unlimited’ model
#       the vectorized code-path is assumed to be profitable while with the
#       ‘dynamic’ model a runtime check guards the vectorized code-path to
#       enable it only for iteration counts that will likely execute faster
#       than when executing the original scalar loop. The ‘cheap’ model
#       disables vectorization of loops where doing so would be cost
#       prohibitive for example due to required runtime checks for data
#       dependence or alignment but otherwise is equal to the ‘dynamic’ model.
#       The default cost model depends on other optimization flags and is
#       either ‘dynamic’ or ‘cheap’. 
#
# INFO: -fsimd-cost-model=model
#       Alter the cost model used for vectorization of loops marked with the
#       OpenMP or Cilk Plus simd directive. The model argument should be one
#       of ‘unlimited’, ‘dynamic’, ‘cheap’. All values of model have the same
#       meaning as described in -fvect-cost-model and by default a cost model
...
#       checks like array bound checks and null pointer checks. This is enabled
#       by default at -O2 and higher. Null pointer check elimination is only
#       done if -fdelete-null-pointer-checks is enabled. 
#
# INFO: -fsplit-ivs-in-unroller
#       Enables expression of values of induction variables in later iterations
#       of the unrolled loop using the value in the first iteration. This
#       breaks long dependency chains, thus improving efficiency of the
#       scheduling passes.
#
#       A combination of -fweb and CSE is often sufficient to obtain the same
#       effect. However, that is not reliable in cases where the loop body is
#       more complicated than a single basic block. It also does not work at
#       all on some architectures due to restrictions in the CSE pass.
#
#       This optimization is enabled by default. 
#
# INFO: -fvariable-expansion-in-unroller
...
#       between -fno-peephole and -fno-peephole2 is in how they are implemented
#       in the compiler; some targets use one, some use the other, a few use
#       both.
#
#       -fpeephole is enabled by default. -fpeephole2 enabled at levels -O2,
#       -O3, -Os. 
#
# INFO: -fno-guess-branch-probability
...
#
# INFO: -freorder-blocks
#       Reorder basic blocks in the compiled function in order to reduce number
#       of taken branches and improve code locality.
#
#       Enabled at levels -O, -O2, -O3, -Os. 
#
# INFO: -freorder-blocks-algorithm=algorithm
#       Use the specified algorithm for basic block reordering. The algorithm
#       argument can be ‘simple’, which does not increase code size (except
#       sometimes due to secondary effects like alignment), or ‘stc’, the
#       “software trace cache” algorithm, which tries to put all often executed
#       code together, minimizing the number of branches executed by making
#       extra copies of code.
#
#       The default is ‘simple’ at levels -O, -Os, and ‘stc’ at levels -O2,
#       -O3. 
#
# INFO: -freorder-blocks-and-partition
#       In addition to reordering basic blocks in the compiled function, in
#       order to reduce number of taken branches, partitions hot and cold basic
#       blocks into separate sections of the assembly and .o files, to improve
...
#       Enabled at levels -O2, -O3, -Os. 
#
# INFO: -fstrict-aliasing
#       Allow the compiler to assume the strictest aliasing rules applicable to
#       the language being compiled. For C (and C++), this activates
#       optimizations based on the type of expressions. In particular, an
#       object of one type is assumed never to reside at the same address as an
#       object of a different type, unless the types are almost the same. For
#       example, an unsigned int can alias an int, but not a void* or a double.
...
#
# INFO: -fstrict-overflow
#       Allow the compiler to assume strict signed overflow rules, depending on
#       the language being compiled. For C (and C++) this means that overflow
#       when doing arithmetic with signed numbers is undefined, which means
#       that the compiler may assume that it does not happen. This permits
#       various optimizations. For example, the compiler assumes that an
#       expression like i + 10 > i is always true for signed i. This assumption
#       is only valid if signed overflow is undefined, as the expression is
#       false if i + 10 overflows when using twos complement arithmetic. When
#       this option is in effect any attempt to determine whether an operation
#       on signed numbers overflows must be written carefully to not actually
#       involve overflow.
#
#       This option also allows the compiler to assume strict pointer
#       semantics: given a pointer to an object, if adding an offset to that
#       pointer does not produce a pointer to the same object, the addition is
#       undefined. This permits the compiler to conclude that p + u > p is
#       always true for a pointer p and unsigned integer u. This assumption is
#       only valid because pointer wraparound is undefined, as the expression
#       is false if p + u overflows using twos complement arithmetic.
#
#       See also the -fwrapv option. Using -fwrapv means that integer signed
#       overflow is fully defined: it wraps. When -fwrapv is used, there is no
#       difference between -fstrict-overflow and -fno-strict-overflow for
...
#       constants, the overflowed value can still be used with -fwrapv, but not
#       otherwise.
#
#       The -fstrict-overflow option is enabled at levels -O2, -O3, -Os. 
#
# INFO: -falign-functions
# INFO: -falign-functions=n
#       Align the start of functions to the next power-of-two greater than n,
#       skipping up to n bytes. For instance, -falign-functions=32 aligns
#       functions to the next 32-byte boundary, but -falign-functions=24 aligns
#       to the next 32-byte boundary only if this can be done by skipping 23
#       bytes or less.
#
# INFO: -fno-align-functions and -falign-functions=1 are equivalent and mean
...
#
# INFO: -fno-align-labels and -falign-labels=1 are equivalent and mean that
...
#
#       If n is not specified or is zero, use a machine-dependent default which
#       is very likely to be ‘1’, meaning no alignment.
#
#       Enabled at levels -O2, -O3. 
#
# INFO: -falign-loops
...
#
# INFO: -fno-align-loops and -falign-loops=1 are equivalent and mean that loops
#       are not aligned.
#
#       If n is not specified or is zero, use a machine-dependent default.
#
...
#       bytes like -falign-functions. In this case, no dummy operations need be
#       executed.
#
# INFO: -fno-align-jumps and -falign-jumps=1 are equivalent and mean that loops
#       are not aligned.
#
#       If n is not specified or is zero, use a machine-dependent default.
#
...
#       bodies are read from these ELF sections and instantiated as if they had
...
#       usual. Since both foo.o and bar.o are merged into a single image, this
#       causes all the interprocedural analyses and optimizations in GCC to
#       work across the two files as if they were a single one. This means, for
#       example, that the inliner is able to inline functions in bar.o into
...
#       into a single GIMPLE representation and optimizes them as usual to
#       produce myprog.
#
#       The only important thing to keep in mind is that to enable link-time
#       optimizations you need to use the GCC driver to perform the link-step.
#       GCC then automatically performs link-time optimization if any of the
#       objects involved were compiled with the -flto command-line option. You
#       generally should specify the optimization options to be used for
#       link-time optimization though GCC tries to be clever at guessing an
#       optimization level to use from the options used at compile-time if you
#       fail to specify one at link-time. You can always override the automatic
#       decision to do link-time optimization at link-time by passing -fno-lto
#       to the link command.
#
...
#       the linker plugin (see -fuse-linker-plugin) passes information to the
#       compiler about used and externally visible symbols. When the linker
#       plugin is not available, -fwhole-program should be used to allow the
#       compiler to make these assumptions, which leads to more aggressive
...
#       -fno-fat-lto-objects is enabled the compile-stage is faster but you
#       cannot perform a regular, non-LTO link on them.
#
...
#
#       This produces individual object files with unoptimized assembler code,
#       but the resulting binary myprog is optimized at -O3. If, instead, the
#       final binary is generated with -fno-lto, then myprog is not optimized.
#
...
#       final object code. GCC automatically selects which files to optimize in
...
#       bytecodes, as they need to be used during the final link stage.
#       Generally options specified at link-time override those specified at
#       compile-time.
#
#       If you do not specify an optimization level option -O at link-time then
#       GCC computes one based on the optimization levels used when compiling
#       the object files. The highest optimization level wins here.
#
#       Currently, the following options and their setting are take from the
#       first object file that explicitely specified it: -fPIC, -fpic, -fpie,
#       -fcommon, -fexceptions, -fnon-call-exceptions, -fgnu-tm and all the -m
#       target flags.
#
...
#       and merged conservatively for conflicting translation units.
...
#       libraries and -lgfortran is added to get the Fortran runtime libraries.
#       In general, when mixing languages in LTO mode, you should use the same
#       link command options as when mixing languages in a regular (non-LTO)
#       compilation.
#
#       If object files containing GIMPLE bytecode are stored in a library
#       archive, say libfoo.a, it is possible to extract and use them in an LTO
#       link if you are using a linker with plugin support. To create static
#       libraries suitable for LTO, use gcc-ar and gcc-ranlib instead of ar and
#       ranlib; to show the symbols of object files with GIMPLE bytecode, use
#       gcc-nm. Those commands require that ar, ranlib and nm have been
#       compiled with plugin support. At link time, use the the flag
#       -fuse-linker-plugin to ensure that the library participates in the LTO
#       optimization process:
#
#              gcc -o myprog -O2 -flto -fuse-linker-plugin a.o b.o -lfoo
#
#       With the linker plugin enabled, the linker extracts the needed GIMPLE
#       files from libfoo.a and passes them on to the running GCC to make them
...
#       program to operate. If the program does not require any symbols to be
#       exported, it is possible to combine -flto and -fwhole-program to allow
#       the interprocedural optimizers to use more aggressive assumptions which
#       may lead to improved optimization opportunities. Use of -fwhole-program
...
#       that is portable between different types of hosts. The bytecode files
#       are versioned and there is a strict version check, so bytecode files
#       generated in one version of GCC do not work with an older or newer
#       version of GCC.
#
...
#       Makefile calling GCC is already executing in parallel. You must prepend
#       a ‘+’ to the command recipe in the parent Makefile for this to work.
#       This option likely only works if MAKE is GNU make. 
#
# INFO: -flto-partition=alg
#       Specify the partitioning algorithm used by the link-time optimizer. The
#       value is either ‘1to1’ to specify a partitioning mirroring the original
#       source files or ‘balanced’ to specify partitioning into equally sized
#       chunks (whenever possible) or ‘max’ to create new partition for every
#       symbol where possible. Specifying ‘none’ as an algorithm disables
#       partitioning and streaming completely. The default value is ‘balanced’.
#       While ‘1to1’ can be used as an workaround for various code ordering
#       issues, the ‘max’ partitioning is intended for internal testing only.
#       The value ‘one’ specifies that exactly one partition should be used
#       while the value ‘none’ bypasses partitioning and executes the link-time
#       optimization step directly from the WPA phase. 
#
# INFO: -flto-odr-type-merging
#       Enable streaming of mangled types names of C++ types and their
#       unification at linktime. This increases size of LTO object files, but
#       enable diagnostics about One Definition Rule violations. 
#
# INFO: -flto-compression-level=n
#       This option specifies the level of compression used for intermediate
#       language written to LTO object files, and is only meaningful in
#       conjunction with LTO mode (-flto). Valid values are 0 (no compression)
#       to 9 (maximum compression). Values outside this range are clamped to
#       either 0 or 9. If the option is not given, a default balanced
#       compression setting is used. 
#
# INFO: -flto-report
#       Prints a report with internal details on the workings of the link-time
#       optimizer. The contents of this report vary from version to version. It
#       is meant to be useful to GCC developers when processing object files in
#       LTO mode (via -flto).
#
#       Disabled by default. 
#
# INFO: -flto-report-wpa
#       Like -flto-report, but only print for the WPA phase of Link Time
#       Optimization. 
#
# INFO: -fuse-linker-plugin
#       Enables the use of a linker plugin during link-time optimization. This
#       option relies on plugin support in the linker, which is available in
...
#       during dynamic linking). Resulting code quality improvements on
#       binaries (and shared libraries that use hidden visibility) are similar
#       to -fwhole-program. See -flto for a description of the effect of this
...
#
# INFO: -fno-fat-lto-objects improves compilation time over plain LTO, but
...
#       Additionally, nm, ar and ranlib need to support linker plugins to allow
#       a full-featured build environment (capable of building static libraries
#       etc). GCC provides the gcc-ar, gcc-nm, gcc-ranlib wrappers to pass the
#       right options to these tools. With non fat LTO makefiles need to be
...
#       -fbranch-probabilities and its related options. Both absolute and
#       relative paths can be used. By default, GCC uses the current directory
#       as path, thus the profile data file appears in the same directory as
#       the object file. 
#
# INFO: -fprofile-generate
# INFO: -fprofile-generate=path
#       Enable options usually used for instrumenting application to produce
#       profile useful for later recompilation with profile feedback based
#       optimization. You must use -fprofile-generate both when compiling and
...
#       -finline-functions, -fipa-cp, -fipa-cp-clone, -fpredictive-commoning,
#       -funswitch-loops, -fgcse-after-reload, and
#       -ftree-loop-distribute-patterns.
#
#       path is the name of a file containing AutoFDO profile information. If
#       omitted, it defaults to fbdata.afdo in the current directory.
#
#       Producing an AutoFDO profile data file requires running your program
#       with the perf utility on a supported GNU/Linux target system. For more
...
#                     --profile=perf.data --gcov=profile.afdo
#
#       The following options control compiler behavior
#       regarding floating-point arithmetic. These options trade off between
...
#
# INFO: -fexcess-precision=style
#       This option allows further control over excess precision on machines
#       where floating-point registers have more precision than the IEEE float
#       and double types and the processor does not support operations rounding
#       to those types. By default, -fexcess-precision=fast is in effect; this
#       means that operations are carried out in the precision of the registers
#       and that it is unpredictable when rounding to the types specified in
#       the source code takes place. When compiling C, if
#       -fexcess-precision=standard is specified then excess precision follows
#       the rules specified in ISO C99; in particular, both casts and
#       assignments cause values to be rounded to their semantic types (whereas
#       -ffloat-store only affects assignments). This option is enabled by
#       default for C if a strict conformance option such as -std=c99 is used.
#
# INFO: -fexcess-precision=standard is not implemented for languages other than
#       C, and has no effect if -funsafe-math-optimizations or -ffast-math is
#       specified. On the x86, it also has no effect if -mfpmath=sse or
...
#
# INFO: -funsafe-math-optimizations
...
#       IEEE or ISO rules/specifications for math functions. It may, however,
#       yield faster code for programs that do not require the guarantees of
#       these specifications. Enables -fno-signed-zeros, -fno-trapping-math,
#       -fassociative-math and -freciprocal-math.
#
#       The default is -fno-unsafe-math-optimizations. 
#
# INFO: -fassociative-math
...
#       overflow (and thus cannot be used on code that relies on rounding
...
#       when both -fno-signed-zeros and -fno-trapping-math are in effect.
#
#       The default is -fno-associative-math. 
#
# INFO: -freciprocal-math
...
#       expressions such as x+0.0 or 0.0*x (even with -ffinite-math-only). This
#       option implies that the sign of a zero result isn't significant.
#
#       The default is -fsigned-zeros. 
#
# INFO: -fno-trapping-math
...
#       all GCC optimizations that are affected by rounding mode. Future
#       versions of GCC may provide finer control of this setting using C99's
#       FENV_ACCESS pragma. This command-line option will be used to specify
#       the default state for FENV_ACCESS. 
#
# INFO: -fsignaling-nans
#       Compile code assuming that IEEE signaling NaNs may generate
#       user-visible traps during floating-point operations. Setting this
#       option disables optimizations that may change the number of exceptions
...
#
# INFO: -fsingle-precision-constant
#       Treat floating-point constants as single precision instead of
#       implicitly converting them to double-precision constants. 
#
# INFO: -fcx-limited-range
...
#       -fbranch-probabilities, to improve optimizations based on the number
#       of times each branch was taken. When a program compiled with
#       -fprofile-arcs exits, it saves arc execution counts to a file called
#       sourcename.gcda for each source file. The information in this data file
#       is very dependent on the structure of the generated code, so you must
#       use the same source code and the same optimization options for both
#       compilations.
#
...
#
# INFO: -fprofile-reorder-functions
#       Function reordering based on profile instrumentation collects first
#       time of execution of a function and orders these functions in ascending
#       order.
#
#       Enabled with -fprofile-use. 
#
# INFO: -fvpt
#       If combined with -fprofile-arcs, this option instructs the compiler to
#       add code to gather information about values of expressions.
#
...
#
# INFO: -fschedule-fusion
#       Performs a target dependent pass over the instruction stream to
#       schedule instructions of same type together because target machine can
#       execute them more efficiently if they are adjacent to each other in the
#       instruction flow.
#
#       Enabled at levels -O2, -O3, -Os. 
#
...
#       -frerun-cse-after-loop, -fweb and -frename-registers. It also turns on
#       complete loop peeling (i.e. complete removal of loops with a small
#       constant number of iterations). This option makes code larger, and may
#       or may not make it run faster.
#
#       Enabled with -fprofile-use. 
#
# INFO: -funroll-all-loops
...
#
# INFO: -fbranch-target-load-optimize2
#       Perform branch target register load optimization after prologue /
#       epilogue threading. 
#
# INFO: -fbtr-bb-exclusive
#       When performing branch target register load optimization, don't reuse
#       branch target registers within any basic block. 
#
# INFO: -fstack-protector
...
#              block being cross-jumped from are matched. The default value is
#              5. 
#
...
#              max-goto-duplication-insns are unfactored. The default value is
#              8. 
#
...
#              computations in the instruction stream. The default value is 20. 
#
...
#              can be applied. The default value is 40. 
#
#       inline-min-speedup
#              When estimated performance improvement of caller + callee
#              runtime exceeds this threshold (in precent), the function can be
#              inlined regardless the limit on --param max-inline-insns-single
...
#              percents. The default value is 100 which limits large function
...
#              example, consider a unit consisting of function A that is inline
#              and B that just calls A three times. If B is small relative to
#              A, the growth of unit is 300\% and yet such inlining is very
...
#              by inlining. The default value is 20 which limits unit growth to
#              1.2 times the original size. Cold functions (either marked cold
#              via an attribute or by profile feedback) are not accounted into
#              the unit size. 
#
#       ipcp-unit-growth
#              Specifies maximal overall growth of the compilation unit caused
...
#              exceeds the given threshold (in percents). The default value is
#              10. 
#
...
#              Specifying 0 disables hoisting of simple expressions. The
#              default value is 10. 
#
...
#              to travel unrestricted distances. The default value is 3. 
#
#       max-hoist-depth
...
#              slow down compilation of huge functions. The default value is
#              30. 
#
...
#              without. The value for compilation with profile feedback needs
#              to be more conservative (higher) in order to make tracer
#              effective. 
#
#       max-cse-path-length
...
#              between collections. Tuning this may improve compilation speed;
...
#              is the smallest of actual RAM and RLIMIT_DATA or RLIMIT_AS. If
#              GCC is not able to calculate RAM on a particular platform, the
#              lower bound of 30% is used. Setting this parameter and
...
#              for equivalent register. Increasing values mean more aggressive
...
#              scheduling. It is a depth of search for available instructions.
...
#              lead to less streams being prefetched
#              (see simultaneous-prefetches). 
#
...
#              default, this should always be 1, which uses a more efficient
#              internal mechanism for comparing types in C++ and Objective-C++.
...
#              only at most the given number of the most frequently-executed
#              loops form regions for regional register allocation. The default
...
#              register allocator instead uses a faster, simpler, and
#              lower-quality algorithm that does not require building a
#              pseudo-register conflict table. The default value of the
#              parameter is 2000. 
#
...
#              is 2, which is the minimal number of registers needed by typical
#              instructions. This value is the best found from numerous
#              experiments. 
#
#       lra-inheritance-ebb-probability-cutoff
#              LRA tries to reuse values reloaded in registers in subsequent
#              insns. This optimization is called inheritance. EBB is used as a
#              region to do this optimization. The parameter defines a minimal
#              fall-through edge probability in percentage used to add BB to
#              inheritance EBB in LRA. The default value of the parameter is
#              40. The value was chosen from numerous runs of SPEC2000 on
#              x86-64. 
#
...
#              for that function is retried without it, after removing all
#              debug insns from the function. If the limit is exceeded even
#              without debug insns, var tracking analysis is completely
#              disabled for the function. Setting the parameter to zero makes
#              it unlimited. 
#
...
#              insns created by -fvar-tracking-assignments, but debug insns may
#              get (non-overlapping) uids above it if the reserved range is
#              exhausted. 
#
...
#              for replacement when compiling for speed
...
#              To avoid exponential effects in the detection of SCoPs, the size
#              of the functions analyzed by Graphite is bounded. The default
...
#              be changed using the loop-block-tile-size parameter. The default
...
#              inner loop) for the -floop-unroll-and-jam. The default value is
#              2. 
#
...
#              C++ name lookup fails for an identifier. The default is 1000. 
#
...
#              aggressive statement sinking. The default value is 75. A small
...
#              if-conversion (-ftree-loop-if-convert) is disabled. The default
#              is 2. 
#
#       allow-store-data-races
#              Allow optimizers to introduce new data races on stores. Set to 1
#              to allow, otherwise to 0. This option is enabled by default at
...
#              the value is 0, use the default for the machine. The default is
#              0. 
#
#       tree-reassoc-width
...
#              accurate register pressure classes. See haifa-sched.c in the GCC
...
#              considered when seeking a basis for a new straight-line strength
#              reduction candidate.
# 
#       asan-globals
...
#              enabled by default when using -fsanitize=address option. To
#              disable use-after-return detection use
...
#              checks. E.g. to disable inline code use
...
#              nubmer of statements in a single generated constructor. Default
#              value is 5000. 
#
